...
class ExecutionHost(object):
  ...
  def updateSudo(self):
    try:
      shell = pxssh.pxssh()
      shell.login(self.address, self.username, self.password, ssh_key = f"{self.LinuxSSHDirectory}id_rsa", login_timeout = 15)
      shell.sendline("sudo visudo")
      shell.flush()
      shell.prompt()
      output = str(shell.before)[2:-5]
      print(output)
...
def main():
  ...
  # try:
  #   command = f"ssh {self.username}@{self.address}"
