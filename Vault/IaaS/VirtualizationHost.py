...
class VirtualizationHost(object):
  def __init__(self, username: str, password: str, root_password: str, private_key: str, address: str, hostname = "VPS-Host"):
  ...
  def updateVirtualizationHost(self):
    ...
    if output == "Done All packages are up to date.":
      return True
    else:
      return False
  ...
  def transferRSAKey(self, background = False):
    ...
    elif self.executing_platform == "Linux":
  ...
  def authorizeRSAKey(self):
    ...
    try:
      command = f"sudo cat /tmp/id_rsa.pub > /home/{self.username}/.ssh/authorized_keys"
      output = (stdin, stdout, stderr) = client.exec_command(command)
  ...
  def windows_createSSHBanner(self):
    banner_location = self.resource_path("assets/banner.txt")
    command = f"pscp -batch -pw {self.root_password} {banner_location} root@{self.address}:/etc/ssh"
    stream = subprocess.run(command)
  ...
  def WindowsRSAKey(self):
    property = "C:\\Users\\Development\\.ssh\\id_rsa.pub"
    return property
  ...
  def preseed_lamp_wordpress(self):
    property = textwrap.dedent(
      ...
      in-target /bin/bash /tmp/provision.sh {self.user} {self.hostname}; \\
      """
    ).strip()
...
def main():
  host = VirtualizationHost("snow", "Kn0wledge!", "fUckS0ci3ty!", "NULL", "192.168.0.1")
